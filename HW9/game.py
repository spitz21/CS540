...
class Teeko2Player:
    ...
    def make_move(self, state):
        ...
        for succ in succs:
            if self.game_value(succ) != 1:
                curr_min = self.min_value(state, 0, float("-inf"), float("inf"))
    ...
    def max_value(self, state, depth, alpha, beta):
        ...
        succs = self.succ(state, self.detect_drop_phase(state), self.my_piece)

        for successor in succs:
            min = self.min_value(successor, depth + 1, alpha, beta)
            alpha = max(alpha, min)
            if alpha >= beta:
                return beta
        return alpha
    ...
    def min_value(self, state, depth, alpha, beta):
        ...
        for succ in succs:
            max = self.max_value(succ, depth + 1, alpha, beta,)
            beta = min(beta, max)
            if alpha >= beta:
                return beta
        return alpha
